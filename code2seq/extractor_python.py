...
parser.add_argument('--data_dir', type=str, default=cwd)
...
parser.add_argument('--output_dir', type=str, default=cwd)
...
def main(path = "Input.py"):
    args = parser.parse_args()
    # parse the given python file
    # change so we can specify the sample file
    out = parse_file(path)
    print(out[0])
    print(type(out))
    # direct_output = subprocess.check_output("python3 python_parser.py " + path, shell=True)
    # direct_output = direct_output.decode("utf-8")
    # print("\n Type Direct Output", type(direct_output))
    # print("\n Direct output", direct_output)
    #
    with open('python_sample.json', 'w') as f:
         json.dump(out, separators=(',', ':'), ensure_ascii=False, fp=f)
    #
    sample_ast = __collect_asts('python_sample.json')
    print("sample ast", sample_ast[0])

    samples = tqdm.tqdm(ast)
